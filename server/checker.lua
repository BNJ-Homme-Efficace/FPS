...
function UpdateChecker(resource)
	if resource and GetResourceState(resource) == 'started' then
		if GetResourceMetadata(resource, 'bnj_checker', 0) == 'yes' then

			local Name = GetResourceMetadata(resource, 'bnj_name', 0)
			local Github = GetResourceMetadata(resource, 'bnj_Github', 0)
			local Version = GetResourceMetadata(resource, 'bnj_version', 0)
...
function Checker()
    ...
    print('^0--------------------------------------------------------------------')
    print("^3Vérifiration des mises à jours disponibles...")
    print('')
    for i, v in pairs(ScriptList) do
        if string.find(v.NewestVersion, v.Version) then
            print('^4'..v.Name..' ('..v.Resource..') ^2✓ ' .. 'À jour - Version ' .. v.Version..'^0')
        else
            print('^4'..v.Name..' ('..v.Resource..') ^1✗ ' .. 'Pas à jour (v'..v.Version..') ^5- Mise à jour trouvée : Version ' .. v.NewestVersion..' ^0('..v.Github..')')
        end
...
function Changelog()
    ...
    for i, v in pairs(ScriptList) do
        if v.Version ~= v.NewestVersion then
            if v.CL then
                print('^3'..v.Resource:upper()..' - Journal des modifications:')
                print('^4'..v.Changelog)
                print('')
            end
        end
